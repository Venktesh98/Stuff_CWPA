#Does the encryption by using SHA256 and generates the key unique
...
# In simple terms, user can use the tool to encrypt their files whatsoever it may be, 
...
from Crypto import Random
loadButton = Button(frame, text="   LOAD FILE   ", command=load_file)
...
password = passInput.get()
encryptButton = Button(frame2, text="       ENCRYPT      ", command=encrypt_the_file)
encryptButton.configure(bg='turquoise4', fg='white', activebackground='coral')
decryptButton = Button(frame2, text="       DECRYPT      ", command=decrypt_the_file)
